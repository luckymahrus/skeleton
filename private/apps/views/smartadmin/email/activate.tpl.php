					...
					<table style="width: 500px; height: 100%;border: 1px solid #ccc; background: #fff;" id="table-content">
						...
						<tr>
							<td align="left" style="vertical-align: top;padding: 30px;">
								...
								<p>&nbsp;</p>
								<p>Je bent bijna klaar om het Zorg Beuningen portaal te kunnen gebruiken.</p>
								<p>Je e-mailadres (<?=$email?>) staat geregistreerd op het portaal.</p>
								<p>&nbsp;</p>
								<p>Klik op de volgende link om jouw Zorg Beuningen account te activeren: <?php echo sprintf(lang('email_activate_subheading'), anchor(links_url(array('class'=>'auth','method'=>'activate')).'/'. $id .'/'. $activation, lang('email_activate_link')));?></p>
								<p>&nbsp;</p>
								<?php if(isset($password) && !empty($password) && !is_null($password)) : ?>
								<p>Jouw tijdelijke wachtwoord is: <span style="font-family: 'Courier New';"><strong><?=@$password?></strong></span></p>
								<p>LET OP! In verband met de veiligheid is het van groot belang om jouw wachtwoord direct te wijzigen na de activatie van jouw account. Dit doe je door op je naam te klikken wanneer je voor het eerst bent ingelogd.</p>
								<?php endif; ?>
								<p>&nbsp;</p>
								<p>&nbsp;</p>
