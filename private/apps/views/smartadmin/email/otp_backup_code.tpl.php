					...
					<table style="width: 500px; height: 100%;border: 1px solid #ccc; background: #fff;" id="table-content">
						...
						<tr>
							<td align="left" style="vertical-align: top;padding: 30px;">
								...
								<p>&nbsp;</p>
								<p>U bent bijna klaar om het Zorg Beuningen portaal te gebruiken.</p>
								<p>Uw e-mailadres (<?=$email?>) geregistreerd op de portal.</p>
								<p>&nbsp;</p>
								<p>Klik op de volgende link om uw Zorg Beuningen account te activeren: <?php echo sprintf(lang('email_activate_subheading'), anchor(links_url(array('class'=>'auth','method'=>'activate')).'/'. $id .'/'. $activation, lang('email_activate_link')));?></p>
								<p>&nbsp;</p>
								<?php if(isset($password) && !empty($password) && !is_null($password)) : ?>
								<p>Uw tijdelijke wachtwoord is: <span style="font-family: 'Courier New';"><strong><?=@$password?></strong></span></p>
								<p>Verander alstublieft uw wachtwoord, zodra u uw account hebt geactiveerd en direct na uw eerste inloggen bij het systeem.</p>
								<?php endif; ?>
								<p>&nbsp;</p>
								<p>&nbsp;</p>
