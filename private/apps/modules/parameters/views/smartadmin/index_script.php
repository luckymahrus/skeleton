	...
	$(document).ready(function()
	{
		...
		dt_<?=$this->router->class?> = $('#dt_<?=$this->router->class?>').dataTable(
		{
		    ...
		    //         	var html = '';
						...
						// {
						//     case 0	: 	html = '<span class="label label-default">Param</span>';	break;
						//     case 1	: 	html = '<span class="label label-success">Config</span>';	break;
						//     default 	: 	html = '<span class="label label-default">Param</span>';	break;
						// }
						// return html;
		    //         }
			   //  },
			    {
			        ...
			        "render": function (data, type, full, meta)
		            {
						...
						switch(parseInt(data))
						{
						    case 0	: 	html = '<span class="label label-default">Yes</span>';	break;
						    case 1	: 	html = '<span class="label label-success">NO</span>';	break;
						    default 	: 	html = '<span class="label label-default">Yes</span>';	break;
						}
						return html;
		            }
			    },
			  ...
			  //   },
				<?php if($allow_detail || $allow_edit || $allow_delete) : ?>
			    {
					...
					"render": function (data, type, full, meta)
		            {
		            	var html = '';
		            	<?php if($allow_detail) : ?>
		            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-success action-detail" data-id="'+full.id+'" data-action="detail" rel="tooltip" data-placement="top" data-original-title="Detail"><i class="fa fa-eye"></i></a>&nbsp;';
		            	<?php endif; if($allow_edit) : ?>
		            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-primary action-edit" data-id="'+full.id+'" data-action="edit" rel="tooltip" data-placement="top" data-original-title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;';
		            	<?php endif; if($allow_delete) : ?>
		            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-danger action-delete" data-id="'+full.id+'" data-action="delete" rel="tooltip" data-placement="top" data-original-title="Delete"><i class="fa fa-trash"></i></a>';
		            	<?php endif; ?>
		...
		$('body').on('click','<?=((isset($actionClass)) ? implode(',',$actionClass) : '')?>', function (e)
		{
			...
			switch(action)
			{
				...
				<?php endif; ?>
				<?php if($allow_delete)	: ?>
				case 'delete' 		: 	URL	= '<?=links_url(array('class'=>$this->router->class,'method'=>'delete'))?>/'+id;						break;
				<?php endif; ?>
			}
