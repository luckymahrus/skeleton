...
<?php endif; ?>	
						<?=form_open_multipart(links_url(array('class'=>$this->router->class,'method'=>$this->router->method)),array('id'=>$this->router->class,'name'=>$this->router->class,'class'=>'smart-form'))?>
							<fieldset>
								...
								<section>
									<label class="label strong"><?=$files['label'].((isset($files['required'])) ? ' <span class="required">(*)</span>' : '')?></label>
									<label class="input input-file<?=(!empty(form_error('files'))) ? ' state-error' : ''?>">
										<span class="button"><input type="file" id="<?=$files['id']?>" name="<?=$files['name']?>" onchange="this.parentNode.nextSibling.value = this.value" accept="<?=$files['accept']?>">Browse</span><input type="text" placeholder="Include some files" readonly="" accept="<?=$files['accept']?>">
									</label>
									<div class="note"></div>
								</section>
							</fieldset>
