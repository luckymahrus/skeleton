	...
	$(document).ready(function()
	{
		...
		dt_<?=$this->router->class?> = $('#dt_<?=$this->router->class?>').dataTable(
		{
			...
			[
			    ...
			    {
			        "data": 'webroutes_title',
			        "searchable": true,
			        "orderable": false,
			        "targets": 2,
			    },
			    {
			        "data": 'webroutes_params',
			        "searchable": true,
			        "orderable": false,
			        "targets": 3,
					"render": function (data, type, full, meta)
		            {
		            	...
		            	{
		            		html = full.webmodules_uri_routes;
		            	}
		            	else
		            	{
			            	if(full.webmodules_id == undefined || full.webmodules_id == null || data == 'default_controller' || data == '404_override' || data == 'translate_uri_dashes')
			            	{
			            		html = data;
			            	}
			            	else
			            	{
			            		html = full.webmodules_uri_routes+'/'+data;
			            	}
		            	}
						return html;
		            }
				...
				<?php if($allow_detail || $allow_edit || $allow_delete) : ?>
			    {
					...
					"render": function (data, type, full, meta)
		            {
		            	var html = '';
		            	<?php if($allow_detail) : ?>
		            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-success action-detail" data-id="'+full.webroutes_id+'" data-webmodulesid="'+full.webmodules_id+'" data-action="detail" rel="tooltip" data-placement="top" data-original-title="Detail"><i class="fa fa-eye"></i></a>&nbsp;';
		            	<?php endif; if($allow_edit) : ?>
		            	if(full.editable == 1 || full.editable == true || full.editable)
		            	{
		            		html += '<a href="javascript:void(0);" class="btn btn-xs btn-primary action-edit" data-id="'+full.webroutes_id+'" data-webmodulesid="'+full.webmodules_id+'" data-action="edit" rel="tooltip" data-placement="top" data-original-title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;';
		            	}
		            	<?php endif; if($allow_delete) : ?>
		            	if(full.status != 1 && (full.removeable == 1 || full.removeable == true || full.removeable))
		            	{
			            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-danger action-delete" data-id="'+full.webroutes_id+'" data-webmodulesid="'+full.webmodules_id+'" data-action="delete" rel="tooltip" data-placement="top" data-original-title="Delete"><i class="fa fa-trash"></i></a>';
		            	}
		            	<?php endif; ?>
