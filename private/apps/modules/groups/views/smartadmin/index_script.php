			...
			[
			    ...
			    },
			    {
					...
					"render": function (data, type, full, meta)
		            {
						...
						switch(data)
						{
						    case '0'	: 	html = '<i class="fa fa-times text-danger"></i>';	break;
						    case 'f'	: 	html = '<i class="fa fa-times text-danger"></i>';	break;
						    case '1'	: 	html = '<i class="fa fa-check text-success"></i>';	break;
						    case 't'	: 	html = '<i class="fa fa-check text-success"></i>';	break;
						    default 	: 	html = '<i class="fa fa-check text-success"></i>';	break;
						}
						return html;
		            }
			    },
			    {
					...
					"render": function (data, type, full, meta)
		            {
						...
						switch(data)
						{
						    ...
						    default 	: 	html = '<span class="label label-success">Active</span>';	break;
						}
						return html;
		            }
			    },
				<?php if($allow_detail || $allow_edit || $allow_delete) : ?>
			    {
					...
					"render": function (data, type, full, meta)
		            {
		            	var html = '';
		            	<?php if($allow_detail) : ?>
		            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-success action-detail" data-id="'+full.groups_id+'" data-action="detail" rel="tooltip" data-placement="top" data-original-title="Detail"><i class="fa fa-eye"></i></a>&nbsp;';
		            	<?php endif; if($allow_edit) : ?>
		            	if(full.groups_name != 'superadmin' && full.groups_name != 'admin')
		            	{
			            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-primary action-edit" data-id="'+full.groups_id+'" data-action="edit" rel="tooltip" data-placement="top" data-original-title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;';
		            	}
		            	<?php endif; if($allow_delete) : ?>
		            	if(meta.settings.jqXHR.responseJSON.recordsTotal > 1 && full.groups_name != 'superadmin' && full.groups_name != 'admin')
		            	{
			            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-danger action-delete" data-id="'+full.groups_id+'" data-action="delete" rel="tooltip" data-placement="top" data-original-title="Delete"><i class="fa fa-trash"></i></a>';
		            	}
		            	<?php endif; ?>
