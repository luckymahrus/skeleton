			...
			[
				...
				<?php if($allow_detail || $allow_edit || $allow_delete) : ?>
			    {
					...
					"render": function (data, type, full, meta)
		            {
		            	var html = '';
		            	<?php if($allow_detail) : ?>
		            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-success action-detail" data-id="'+full.webthemes_id+'" data-action="detail" rel="tooltip" data-placement="top" data-original-title="Detail"><i class="fa fa-eye"></i></a>&nbsp;';
		            	<?php endif; if($allow_edit) : ?>
		            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-primary action-edit" data-id="'+full.webthemes_id+'" data-action="edit" rel="tooltip" data-placement="top" data-original-title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;';
		            	<?php endif; if($allow_delete) : ?>
		            	if(full.status != 1)
		            	{
			            	html += '<a href="javascript:void(0);" class="btn btn-xs btn-danger action-delete" data-id="'+full.webthemes_id+'" data-action="delete" rel="tooltip" data-placement="top" data-original-title="Delete"><i class="fa fa-trash"></i></a>';
		            	}
		            	<?php endif; ?>
